...
namespace HashAnalyser.Training
{
    ...
    /// </summary>
    public class MulticlassHashAnalysisTrainer : HashAnalysisTrainer
    {

        /// <summary> 
        /// Class for training a new binary model using the device specified in <paramref name="deviceType"/>. Default is CUDA.
        /// </summary>
        public MulticlassHashAnalysisTrainer(DeviceType deviceType = DeviceType.CUDA) : base(deviceType) { }


        public override void TrainModel(string dataSetFileName)
        {
            var trainer = MulticlassHashTransformer.GetModel(_mlContext);
        ...
        protected override void Evaluate(ITransformer model, IDataView data)
        {
            ...
            MulticlassClassificationMetrics metrics = _mlContext.MulticlassClassification.Evaluate(transformedTest);
